...
def infer_hf_api(prompt: str, model: str, temperature: float, max_tokens: int, endpoint: str | None = None) -> str:
    ...
    if not hf_token:
        raise RuntimeError("HF_TOKEN is not set. Add it to att1/.env or export it in your shell.")
    ...
    # temperature=0 is invalid for HF inference API; use greedy decoding instead.
    params: dict = {"max_new_tokens": max_tokens, "return_full_text": False}
    if temperature > 0:
    ...
    for url in urls:
        ...
        if r.status_code in (400, 404, 422, 503) and not endpoint:
            skipped.append(f"{url} -> {r.status_code}: {r.text[:200].strip()}")
            print(f"  [HF] Skipping {url}: {r.status_code} {r.text[:120].strip()}")
            continue
        if r.status_code == 429:
            raise RuntimeError(f"HF rate limit (429) at {url}. Wait and retry, or use a different model.")
        r.raise_for_status()
        obj = r.json()
        ...
        raise RuntimeError(f"Unexpected HF response format: {obj}")
    raise RuntimeError(
        f"All HF endpoints failed for model: {model}\n"
